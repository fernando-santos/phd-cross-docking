...
ILOINCUMBENTCALLBACK2(solIntCallBack, ModeloBC&, modBC, Grafo*, g){
	...
	if ( getObjValue() < (modBC.valorSubtrair - 0.01 ) )
	{
		...
		do
		{
			r->inserirVerticeFim(indiceVertDual[proxVertice]);
		...
		r->inserirVerticeFim(indiceVertDual[zeroLinha]);
		r->setCustoRota(g);
		...
		if (conjW.size() > 0){
			...
			for (int j = 0; j < tam; ++j){
				...
				if (conjW[j] == 0){ //arestas partindo do deposito artificial (o vertice 0 sempre esta em W, mas nao necessariamente esta em algum corte)
					...
					{
						if ( g->existeArestaDual( conjW[j] , x ) )
						{
							inW = false;
							for (int y = 0; y < tam; ++y)
							{
...
ModeloBC::ModeloBC(Grafo* g, float* custoCD, float valSub){
	model = IloModel(env);
...
void ModeloBC::setObjectiveFunction( Grafo *g, float* custoCD ){
	IloExpr obj(env);
	...
	for(int i = 1; i <= nRequests; ++i) obj += custoCD[i] * t[i];
	obj += MAIS_INFINITO * y[depositoArtificial];
	obj += MAIS_INFINITO * y[0];
	obj += MAIS_INFINITO * t[0];

	model.add(IloObjective(env, obj));
}

void ModeloBC::setConstraints1e2(Grafo* g){
	int depositoArtificial = nVertices+1;
	IloExpr expr = IloExpr(env);
	for (int j = 1; j <= nRequests; ++j){
		if ( g->existeArestaDual( 0 , j ) ) expr += x[0][j];
	}
	model.add(expr == 1);
	expr.end();	
	...
	for (int i = (nRequests+1); i <= nVertices; ++i){
		if ( g->existeArestaDual( i , depositoArtificial ) ) expr2 += x[i][depositoArtificial];
	}
	model.add(expr2 == 1);
	expr2.end();	
}

void ModeloBC::setConstraints3e4( Grafo* g ){
	for (int i = 1; i <= nVertices; ++i){
		IloExpr expr = IloExpr(env);
		for (int h = 0; h <= nVertices; ++h){
			if ( g->existeArestaDual( h , i ) ) expr += x[h][i];
		}
		expr -= y[i];
		model.add(expr == 0);
		expr.end();
		...
		for (int j = 1; j <= (nVertices+1); ++j){
			if ( g->existeArestaDual( i , j ) ) expr2 += x[i][j];
		}
		expr2 -= y[i];
		model.add(expr2 == 0);
		expr.end();
...
void ModeloBC::setConstraints5e6(Grafo* g){
	IloExpr expr = IloExpr(env);
	...
	IloExpr expr2 = IloExpr(env);
	for (int i = nRequests+1; i <= nVertices; ++i) expr2 += g->getCargaVertice(i) * y[i];
	model.add( expr2 <= g->getCapacVeiculo() );
	expr2.end();
}

void ModeloBC::setConstraints7(){
	for (int i = 1; i <= nRequests; ++i){
		model.add(t[i] + y[i] - y[nRequests+i] >= 0);
	}
}

void ModeloBC::calculaCaminhoElementar(Grafo* g, int tempo){
