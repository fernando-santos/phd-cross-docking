...
int GRASP::run(int it_max, float valorSub, Rota* rotaCplex){
	...
	if (rotaCplex != NULL){ //vai explorar a rota obtida pelo branch and cut do cplex
		...
		if (rCplex->custo < (rotasNegativas[0]->custo - 0.01)) insereRotaNegativa(rotasNegativas, rCplex, 'C');
		guiding = rCplex;
	...
	while(it <= it_max){
		...
		if (r->custo < (valorSub - 0.01)) insereRotaNegativa(rotasNegativas, r, 'C');
		...
		{ 
			...
			if (guiding->custo < (valorSub - 0.01)) insereRotaNegativa(rotasNegativas, guiding, 'C');
		}
