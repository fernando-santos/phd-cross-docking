...
typedef struct strLabel
{
	...
	strLabel(unsigned long long int vertVisF, unsigned long long int vertVisF2, unsigned long long int vertVisC, int numVisitF, int numVisitC, int ult, int antec, int carga, float custo, ptrLabel p, ptrLabel px) : vertVisitadosF(vertVisF), vertVisitadosF2(vertVisF2), vertVisitadosC(vertVisC), numVisitadosF(numVisitF), numVisitadosC(numVisitC), ultimoVertice(ult), verticeAntecessor(antec), cargaAcumulada(carga), custoDual(custo), pai(p), prox(px){}
} Label;
