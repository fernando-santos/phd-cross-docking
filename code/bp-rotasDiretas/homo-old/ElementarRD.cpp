...
ElementarRD::ElementarRD(Grafo* g, float valorSub) : valorSubtrair(valorSub), menorLabelObtido(NULL), menorCustoObtido(MAIS_INFINITO){
	lixo = NULL;
	numCommodities = g->getNumReqsDual();
	...
	for (int i = 0; i <= numCommodities; ++i) verticesCoding[i] = pow (2, i);
}
...
int ElementarRD::calculaCaminhoElementar(Grafo* g, int tempo){
	int cargaMaxima = g->getCapacVeiculo();
	int depositoArtificial = 2*numCommodities+1;
	...
	while(menorIndice > 0)
	{
		...
		for (int i = 1; i <= numCommodities; ++i)
		{
			...
			if ( ( ( it->vertVisitados2 & verticesCoding[i] ) == 0 ) && ( carga <= cargaMaxima ) )
			{
				...
				{
					if ( ( ( conjVisitados2 & verticesCoding[z] ) == 0 )  && ( carga + g->getCargaVertice(indiceVertDual[z]) > cargaMaxima ) )
					{
	...
	for ( int i = 1; i <= numCommodities; ++i)
	{
		...
		while( it != NULL )
		{
			...
			{
				if ( ( it->vertVisitados1 & verticesCoding[j-numCommodities] ) > 0 )
				{
					numVisitados1 = 1;
	...
	if (menorCustoObtido < ( valorSubtrair - 0.01 ) )
	{
		return 1;
	}
	else
...
Rota** ElementarRD::getRotaCustoMinimo(Grafo* g, float percentual){
	float limiteAceitacao, custoR;
	int depositoArtif = 2*numCommodities+1;
	...
	if (limiteAceitacao > (valorSubtrair - 0.01))
	{
		limiteAceitacao = valorSubtrair - 0.01;
	}
	...
	{
		Rota** r = new Rota*[numRotasRetornadas+1]; numRotasRetornadas = 0;
		for (int i = numCommodities+1; i < depositoArtif; ++i)
		{
			aux = vetorLabels[i].cabeca;
			while (aux != NULL)
			{
				...
				if ( ( aux->vertVisitados1 == aux->vertVisitados2 ) && ( custoR < limiteAceitacao ) )
				{
					...
					r[numRotasRetornadas]->inserirVerticeFim(indiceVertDual[depositoArtif]);
					tmp = aux;
					while (tmp != NULL)
					{
