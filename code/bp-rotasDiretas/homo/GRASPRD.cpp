...
int GRASPRD::run(int it_max, float valorSub)
{
	...
	while( it < it_max )
	{
		...
		if ( r->custo < ( valorSub - 0.01 ) ) insereRotaNegativa( r );
		else delete r;
	}
...
void GRASPRD::insereRotaNegativa( RotaRD* r )
{
	...
	{
		iguais = false;
		if ( r->carga == rotasNegativas[i]->carga ) //eh igual na carga, mas pode nao ser igual no custo
		{
	...
	if ( i == tam ) rotasNegativas.push_back(r);
	else delete r;
}
...
Rota* GRASPRD::getRotaConvertida(int index)
{
	...
	float custo = g->getCustoAresta(0, rotasNegativas[index]->vertices[1], true);
	for (int i = 1; i <= ultimo; ++i) custo += g->getCustoAresta(rotasNegativas[index]->vertices[i], rotasNegativas[index]->vertices[i+1], true);
