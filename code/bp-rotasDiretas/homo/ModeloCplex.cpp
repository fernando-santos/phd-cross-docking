...
ModeloCplex::~ModeloCplex()
{
	...
	for (int r = 0; r < qRotasD; ++r)
	{
		delete [] d_ir[r];
		if (ptrRotasD[r]->decrNumApontadores()) delete ptrRotasD[r];
	}
	for ( int r = 0; r < maxVeic; ++r ) if (melhoresRotas[r]->decrNumApontadores()) delete melhoresRotas[r];

	delete [] melhoresRotas;
	delete [] xiPtInterior;
	a_ir.clear();
	d_ir.clear();
	ptrRotas.clear();
...
: env(), modelPrimal(env), cplexPrimal(modelPrimal),  modelDual(env), cplexDual(modelDual),
  maxVeic(maxVeiculos), nRequests(g->getNumReqs()), custoTrocaCD(custoTroca), nPontosInteriores(nPInteriores){
	...
	melhoresRotas = new Rota*[maxVeic];
...
void ModeloCplex::constroiModeloDual(){
	...
	//Restricoes - Constraints 3
	//( Nao serao inicializadas, apenas receberao constraints ao longo do programa com as rotas diretas )
	constraintsD3 = IloRangeArray(env);
}


void ModeloCplex::updateDualCosts(Grafo* g, int* vetReqsPD, int nReqsPD){
...
void ModeloCplex::updateDualCostsRD(Grafo* g, int* vetReqsCD, int nReqsCD){
	//insere nos vertices do grafo os valores das variaveis duais theta
	IloNumArray dualValuesConstraintsP2(env);
	cplexPrimal.getDuals(dualValuesConstraintsP2, constraintsP2);
	for (int i = 1; i <= nRequests; ++i)
	{
		g->setCustoVerticeDual(i, -dualValuesConstraintsP2[i-1]-dualValuesConstraintsP2[nRequests+i-1]);
		g->setCustoVerticeDual(nRequests+i, 0);
	}
...
void ModeloCplex::insert_ColumnPrimal_RowDual_Direct(Rota* r){
	//Armazena a rota na matriz de rotas, que sera usada posteriormente
	short int* rota = new short int[2*nRequests+1];
	memset(rota, 0, (2*nRequests+1) * sizeof(short int));
	d_ir.push_back(rota);
	ptrRotasD.push_back(r);
	r->incrNumApontadores();

	vector <int> vertRota = r->getVertices();
	int numVertAtualizar = vertRota.size()-1;
	for(int j = 1; j < numVertAtualizar; ++j){
		++d_ir[qRotasD][vertRota[j]]; //+1 apenas nas posicoes que o vertice esteja na rota
	}
	...
	for (int i = 1; i <= 2*nRequests; i++){
		if (d_ir[qRotasD][i] > 0){
			col += constraintsP2[i-1](d_ir[qRotasD][i]);
			res += d_ir[qRotasD][i] * theta[i];
		}
	}

	char nome[20];
	sprintf(nome, "del%d", qRotasD);
	delta.add(IloNumVar(col, 0, 1, ILOFLOAT, nome));
	col.end();
	constraintsD3.add(res <= r->getCusto());
	modelDual.add(constraintsD3[qRotasD]);
	res.end();
...
void ModeloCplex::imprimeRotasBasicas(){
	...
	cplexPrimal.getValues(values, delta);
	for (int r = 0; r < qRotasD; ++r)
	{
		if (values[r] >= 0.00001)
		{
			printf("delta[%d](%f): ", r, values[r]);
			ptrRotasD[r]->imprimir();
		}
	}
	values.end();
}

void ModeloCplex::imprimeMelhoresRotas(Grafo* g){
	int i, j;
	bool direta;
	vector<int> v;
	float custoRotaCD;
	...
	for ( int k = 0; k < maxVeic; ++k )
	{
		...
		if ( i == ( 2*j + 1 ) )
		{
			custoRotaCD = g->getCustoAresta(v[0], v[1]);
			for ( i = 1; i < ( v.size()-1 ); ++i )
			{
				custoRotaCD += g->getCustoAresta(v[i], v[i+1]);
				if ( v[i] > nRequests )
				{
					j = 1;
					direta = false;
					...
					if ( !direta )
					{
						printf("C "); melhoresRotas[k]->imprimir(); 
						break;
					}
				}
			}
			if ( direta )
			{
				if ( melhoresRotas[k]->getCusto() < ( custoRotaCD - 0.00001 ) ) { printf("D "); melhoresRotas[k]->imprimir(); } 
				else { printf("C "); melhoresRotas[k]->imprimir(); }
			}
		}
		else { printf("C "); melhoresRotas[k]->imprimir(); }
	}
