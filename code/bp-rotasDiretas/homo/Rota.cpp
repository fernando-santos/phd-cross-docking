...
void Rota::setCustoRota(Grafo* G, bool RD){
	int tam = vertices.size();
	custo = G->getCustoAresta(0, vertices[1], RD);
	for (int i = 2; i < tam; ++i){
		custo += G->getCustoAresta(vertices[i-1], vertices[i], RD);
	}
}
