...
ElementarRDH::ElementarRDH(Grafo* g, float valorSub) : valorSubtrair(valorSub), menorLabelObtido(NULL), menorCustoObtido(MAIS_INFINITO){
...
int ElementarRDH::calculaCaminhoElementar(Grafo* g, int classe){
	...
	while(menorIndice > 0)
	{
		...
		for (int i = 1; i <= numCommodities; ++i)
		{
			...
			if ( ( ( it->vertVisitados2 & verticesCoding[i] ) == 0 ) && ( carga <= cargaMaxima ) )
			{
				...
				if ( !verificaLabelDominadoEDominaF( g, conjVisitados2, numVisitados2, numVisitados1, i, custo ) )
				{
...
bool ElementarRDH::verificaLabelDominadoEDominaF(Grafo* g, unsigned long long int visit, int numVisit, int numVisitC, int ult, float custo ){
	int domCusto;
	...
	while (it != NULL) //Verifica a dominancia para todos os labels (o novo label so sera inserido se nao houver nenhum q o domina - ou seja igual)
	{
		...
		itUpdate = true;
		if ( ( numVisit < it->numVertVisitados2 ) && ( domCusto >= 0 ) ) //Novo Label DOMINA parcialmente
		{
			domVisit = true;
			for ( int j = 1; j <= numCommodities; ++j )
			{
				if ( ( it->vertVisitados2 & verticesCoding[j] ) < ( visit & verticesCoding[j] ) )
				{
					domVisit = false;
					break;
				}
			}
			if ( ( domVisit ) && ( numVisitC <= it->numVertVisitados1 ) )//Novo Label domina TOTAL o label armazenado (que sera excluido)
			{
		...
		else if ( ( numVisit > it->numVertVisitados2 ) && ( domCusto <= 0 ) ) //Label armazenado DOMINA parcialmente
		{
			domVisit = true;
			for ( int j = 1; j <= numCommodities; ++j )
			{
				if ( ( visit & verticesCoding[j] ) < ( it->vertVisitados2 & verticesCoding[j] ) )
				{
					domVisit = false;
					break;
				}
			}
			if ( ( domVisit ) && ( it->numVertVisitados1 <= numVisitC ) ) return true; //Label armazenado domina TOTAL o novo label
		}
		else if ( visit == it->vertVisitados2 )
		{
			if ( ( domCusto <= 0 ) && ( it->numVertVisitados1 <= numVisitC ) )
			{
				return true;
			}
			else if ( ( domCusto >= 0 ) && ( numVisitC <= it->numVertVisitados1 ) )
			{
...
bool ElementarRDH::verificaLabelDominadoEDominaC(Grafo* g, unsigned long long int visit1, unsigned long long int visit2, 
...
Rota** ElementarRDH::getRotaCustoMinimo(Grafo* g, int classe, float percentual){
...
void ElementarRDH::imprimeLabel(ptrLabelRD aux){
...
char* ElementarRDH::decToBin(unsigned long long int dec){
	char tmp[70];
