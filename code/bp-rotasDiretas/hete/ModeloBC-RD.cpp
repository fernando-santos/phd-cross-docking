...
ModeloBC::ModeloBC(Grafo* g, int k, float valSub){
	veic = k;
	model = IloModel(env);
...
void ModeloBC::setConstraints1e2(Grafo* g){
	...
	for (int j = 1; j <= nRequests; ++j){
		if ( g->existeArestaRD( 0 , j ) ) expr += x[0][j];
	}
	model.add(expr == 1);
	expr.end();	
	...
	for (int i = (nRequests+1); i <= nVertices; ++i){
		if ( g->existeArestaRD( i , depositoArtificial ) ) expr2 += x[i][depositoArtificial];
	}
	model.add(expr2 == 1);
	expr2.end();	
}
...
void ModeloBC::setConstraints3e4( Grafo* g ){
	for (int i = 1; i <= nVertices; ++i){
		...
		for (int h = 0; h <= nVertices; ++h){
			if ( g->existeArestaRD( h , i ) ) expr += x[h][i];
		}
		expr -= y[i];
		model.add(expr == 0);
		expr.end();
		...
		for (int j = 1; j <= (nVertices+1); ++j){
			if ( g->existeArestaRD( i , j ) )	expr2 += x[i][j];
		}
		expr2 -= y[i];
		model.add(expr2 == 0);
		expr.end();
...
void ModeloBC::setConstraints6e7(Grafo* g){
	for (int i = 1; i <= nRequests; ++i){
		model.add(y[i] - y[nRequests+i] == 0);
	}	

	IloExpr expr = IloExpr(env);
	for (int i = 1; i <= nRequests; ++i) expr += g->getPesoCommodity(i) * y[i];
	model.add( expr <= g->getCapacVeiculo( veic ) );
	expr.end();
}
...
void ModeloBC::calculaCaminhoElementar(Grafo* g, int tempo){
    ...
    int timeLimit = 10800 - ( time(0) - tempo );
