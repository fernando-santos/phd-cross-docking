...
bool NoArvoreArcos::executaBranchingArcos(ModeloCplex* ptrMod, Grafo* g, char** varComRestr, int veic, int commod, char opCam, long int tempo){
	...
	ptrNoArcos noAtual = new NoArvoreArcos();
	float lDual, lPrimal, menorLimiteDual;
	ptrNoArcos aux, listaNosAtivos = NULL;

	while (noAtual != NULL) {
		//inicializa o primeiro filho, fixa o arco em 0 e chega na raiz
		mCplex->limpaVariaveisArvoreB2();
		aux = new NoArvoreArcos(noAtual, 0);
		...
		mCplex->limpaVariaveisArvoreB2();
		aux = new NoArvoreArcos(noAtual, 1);
		if (aux->alcancaRaizNoSegundoBranching(g, opCam)) {
			aux->limiteDual = mCplex->cplexPrimal.getObjValue();
			if (aux->limiteDual < ModeloCplex::limitePrimal) {
				if (aux->procuraArcoMaisViolado() > 0) {
		...
		if (listaNosAtivos != NULL) //procura pelo no com menor limite dual (best bound strategy)
		{
			int nosArmazenados = 0;
			aux = listaNosAtivos;
			ptrNoArcos bestAnterior, anterior = NULL;
			menorLimiteDual = ModeloCplex::limitePrimal + 1;
			while ( aux != NULL )
			{
				if ( aux->limiteDual < menorLimiteDual )
				{
					noAtual = aux;
					bestAnterior = anterior;
					menorLimiteDual = aux->limiteDual;
				}
				anterior = aux;
				aux = aux->prox;
				++nosArmazenados;
			...
			//apos ter passado por todos os nos da lista de nos ativos, atualiza a lista retirando o menor no
			if ( bestAnterior != NULL ) bestAnterior->prox = noAtual->prox;
			else listaNosAtivos = noAtual->prox;
			
			lDual = noAtual->limiteDual;
			lPrimal = ModeloCplex::limitePrimal;
			printf(" %d\t\t  %d\t\t %0.3f\t\t %0.3f\t\t%0.2f\t\t%ld\n", ++totalNosCriados, nosArmazenados, lDual, lPrimal, ((lPrimal - lDual) / lPrimal) * 100, time(0)-tempo);
		}
	}
	return bestPrimal;
}
...
bool NoArvoreArcos::alcancaRaizNoSegundoBranching(Grafo* G, char opCaminho) {
	...
	do{
		...
		for (int k = iVeicForn; k < limVeic; ++k, ++iVeicForn){
			...
			colunas = retornaColuna(G, (k % maxV), true, true, mCplex->cplexPrimal.getDual(mCplex->constraintsPrimal1[(k % maxV)]), opCaminho, temp);
			if (colunas != NULL){
		...
		for (int k = iVeicCons; k < limVeic; ++k, ++iVeicCons){
			...
			colunas = retornaColuna(G, (k % maxV), false, true, mCplex->cplexPrimal.getDual(mCplex->constraintsPrimal2[(k % maxV)]), opCaminho, temp);
			if (colunas != NULL){
				for (int i = 0; colunas[i] != NULL; ++i){
