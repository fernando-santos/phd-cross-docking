...
ModeloCplexI::ModeloCplexI(Grafo* g, int numCommodities, int maxVeiculos, int custoTroca, char* arqSaida) : 
...
void ModeloCplexI::initVars(){
	...
	for (int k = 0; k < maxVeic; ++k){
		x[k] = IloArray <IloIntVarArray> (env, nVertices);
		for(int i = 0; i < nVertices; ++i){
			x[k][i] = IloIntVarArray(env, nVertices, 0, 1);
			for (int j = 0; j < nVertices; ++j){
				char nomeVar[20];
	...
	y = IloArray <IloIntVarArray> (env, nVertices);
	for (int q = 0; q < nVertices; ++q){
		y[q] = IloIntVarArray(env, maxVeic, 0, 1);
		for(int k = 0; k < maxVeic; ++k){		
			char nomeVar[20];
	...
	u = IloArray<IloIntVarArray>(env, nCommodities+1);
	for (int q = 0; q <= nCommodities; ++q){
		u[q] = IloIntVarArray(env, maxVeic, 0, 1);
		for(int k = 0; k < maxVeic; ++k){
			char nomeVar[20];
...
void ModeloCplexI::setObjectiveFunction(Grafo *g, int custoTroca){
	IloExpr obj(env);
	float custo;
...
void ModeloCplexI::setConstraints2(){
	IloRangeArray constr = IloRangeArray(env, maxVeic*nVertices);
	int c = 0;
...
void ModeloCplexI::setConstraints3(Grafo* g){
...
void ModeloCplexI::setConstraints5(){
...
void ModeloCplexI::setConstraints6(){
	IloRangeArray constr = IloRangeArray(env, maxVeic);
...
void ModeloCplexI::setConstraints8(Grafo* g){
	IloRangeArray constr = IloRangeArray(env, nVertices);
	int c = 0;
...
void ModeloCplexI::setConstraints9(Grafo* g){
	IloRangeArray constr = IloRangeArray(env, maxVeic);
...
void ModeloCplexI::setConstraints10(Grafo* g){
	IloRangeArray constr = IloRangeArray(env, maxVeic);
...
void ModeloCplexI::setConstraints11(){
...
void ModeloCplexI::setConstraints12(){
	IloRangeArray constr = IloRangeArray(env, maxVeic*nCommodities);
	int c = 0;
