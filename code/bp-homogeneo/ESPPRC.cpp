...
/*						//Antes de executar o teste da dominancia, marco aqueles vertices 'unreachable'
						...
						{
							if ( ( ( conjVisitadosF & verticesCoding[j] ) == 0 ) && ( ( carga + g->getCargaVertice(j) ) > cargaMaxima ) )
							{
...
*/					}
					else
					{
						...
						if ( ( conjVisitadosF2 & verticesCoding[i-nRequests] ) == 0 ) custo += vetorXi[i-nRequests];
		...
		for ( int i = nRequests+1; i < depositoArtif; ++i )
		{
			...
			while ( aux != NULL )
			{
				...
				if ( custoR < limiteAceitacao )
				{
					...
					rr[numRotasRetornadas]->inserirVerticeFim(depositoArtif);
					tmp = aux;
					while ( tmp != NULL )
					{
						rr[numRotasRetornadas]->inserirVerticeInicio(tmp->ultimoVertice);
						tmp = tmp->pai;
					}
					rr[numRotasRetornadas]->inserirVerticeInicio(0);
					rr[numRotasRetornadas]->setCustoReduzido(custoR);
					rr[numRotasRetornadas]->setCustoRota(g);
					++numRotasRetornadas;
				}
				aux = aux->prox;
			}
