...
int GRASP::insercao(RotaG* r, int i){
	...
	if ( ( itr[1] == 3 ) && ( itr[2] == 8 ) && ( itr[3] == 7 ) ) printf("3!\n");
...
int GRASP::swap(RotaG* r){
	...
	for ( int i = 1; i < inicioCons; ++i )
	{
		for ( int j = i+1; j < inicioCons; ++j )
		{
			...
			if ( entra < ( sai - 0.001 ) ) //troca i e j na rota, atualiza seu custo e executa nova iteracao
			{
				tmp = r->vertices[i];
...
//					else
//					{
//						if ( it == 3 ) { printf("S"); r->imprimir(); }
//					}
				}
//				else
//				{
//					if ( it == 3 ) { printf("I"); r->imprimir(); }
//				}
			}
//			else
//			{
//				if ( it == 3 ) { printf("R"); r->imprimir(); }
//			}
		}
...
void GRASP::insereRotaNegativa(vector < RotaG* >& v, RotaG* r, char origem){
	...
	while(++i < tam){
		iguais = false;
		if ((r->cargaForn == v[i]->cargaForn) && (r->cargaCons == v[i]->cargaCons)){ //eh igual na carga, mas pode nao ser igual no custo
			if ((r->custo < (v[i]->custo + 0.001)) && (r->custo > (v[i]->custo - 0.001))){ // eh igual no custo tambem, mas pode nao ser na rota
...
Rota* GRASP::getRotaConvertida(int index){
	...
	float custo = g->getCustoAresta(0, rG->vertices[1]);
	for (int i = 1; i <= ultimo; ++i){
		custo += g->getCustoAresta(rG->vertices[i], rG->vertices[i+1]);
	}
