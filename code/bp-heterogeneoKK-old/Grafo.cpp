...
Grafo::Grafo(char* instanciaSolomon, int nCmdt) : numCmdt(nCmdt), numVertices(2*nCmdt + 2){
	...
	inFile >> tmp >> capacVeiculo[0]; //Le a capacidade do veiculo
	capacVeiculo[1] = 1.5 * capacVeiculo[0];
	capacVeiculo[2] = 0.5 * capacVeiculo[0];
	...
	vert[0]->custoArestas[0][0] = MAIS_INFINITO;
	vert[0]->custoArestas[0][numVertices-1] = MAIS_INFINITO;
	vert[numVertices-1]->custoArestas[0][numVertices-1] = MAIS_INFINITO;
	vert[numVertices-1]->custoArestas[0][0] = MAIS_INFINITO;
	for (int j = 1; j < numVertices-1; ++j){
		//custo do deposito aos demais vertices e vice-versa
		vert[0]->custoArestas[0][j] = sqrt (pow ((matriz[0][0] - matriz[j][0]), 2) + pow ((matriz[0][1] - matriz[j][1]), 2));
		vert[j]->custoArestas[0][numVertices-1] = vert[0]->custoArestas[0][j];

		//o custo para chegar ao deposito, ou sair do deposito artificial eh MAIS_INFINITO
		vert[j]->custoArestas[0][0] = MAIS_INFINITO;
		vert[numVertices-1]->custoArestas[0][j] = MAIS_INFINITO;
	}
	...
	for (int i = 1; i < numVertices-1; ++i){
		if (i <= nCmdt){
			for (int j = 1; j < numVertices-1; ++j){
				if ((j <= nCmdt) && (i != j)){
					vert[i]->custoArestas[0][j] = sqrt (pow ((matriz[i][0] - matriz[j][0]), 2) + pow ((matriz[i][1] - matriz[j][1]), 2));
				}else{
					vert[i]->custoArestas[0][j] = MAIS_INFINITO;
				}
			}
		}else{
			for (int j = 1; j < numVertices-1; ++j){
	...
	//USA OS VALORES DAS ARESTAS PARA A CLASSE '=' {0} E CALCULA OS CUSTOS DAS ARESTAS PARA AS CLASSES '+' {1} E '-' {2}
	for ( int i = 0; i < numVertices; ++i )
	{
		for ( int j = 0; j < numVertices; ++j )
		{
			if ( vert[i]->custoArestas[0][j] < MAIS_INFINITO )
			{
				vert[i]->custoArestas[1][j] = 1.5 * vert[i]->custoArestas[0][j];
				vert[i]->custoArestas[2][j] = 0.5 * vert[i]->custoArestas[0][j];
			}
			else
			{
				vert[i]->custoArestas[1][j] = MAIS_INFINITO;
				vert[i]->custoArestas[2][j] = MAIS_INFINITO;
			}
		}
	}
}
