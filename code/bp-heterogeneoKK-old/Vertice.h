...
class Vertice{
	public:
		//destrutor
		~Vertice(){
			for ( int cl = 0; cl < 3; ++cl ) delete custoArestas[cl];
		...
		Vertice(int n) : numArestas(n){
			custoArestas = new float*[3]; //representam as 3 classes de veiculos heterogeneos
			for ( int cl = 0; cl < 3; ++cl ) custoArestas[cl] = new float[numArestas];
