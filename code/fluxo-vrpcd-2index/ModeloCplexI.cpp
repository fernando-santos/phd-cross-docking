...
ModeloCplexI::ModeloCplexI(Grafo* g, int numCommodities, int maxVeiculos, int custoCD, int janelaCD, char* arqSaida, float* classesV) : 
...
void ModeloCplexI::initVars(){
	...
	u = IloArray<IloIntVarArray>(env, maxVeic);
	for (int q = 0; q < maxVeic; ++q){
		u[q] = IloIntVarArray(env, nCommodities+1, 0, 1);
...
void ModeloCplexI::setObjectiveFunction(Grafo *g, int custoCrossDocking){
	IloExpr obj(env);
	float custo;
...
void ModeloCplexI::setConstraints9(){
...
void ModeloCplexI::setConstraints10(){
	IloRangeArray constr = IloRangeArray(env, maxVeic*nCommodities);
	int c = 0;
...
void ModeloCplexI::setConstraintsJanela(Grafo* g, int janelaCD){
...
float ModeloCplexI::optimize(int timeLimit){
