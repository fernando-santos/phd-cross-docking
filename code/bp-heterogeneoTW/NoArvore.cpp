...
int NoArvore::tempoCrossDocking;
...
NoArvore::NoArvore(ModeloCplex& mCplex, int tempoCD, int nPontosExt){
...
bool NoArvore::alcancaRaizNo(ModeloCplex& mCplex, Grafo* G, char opcaoCaminho){
	Rota** colunas;
	float valorSubtrair;
	bool alcancouRaiz = false;
	int k, limVeic, iVeicForn = 0, iVeicCons = 0;

	//Obtem as janelas de tempo do deposito, fornecedores e consumidores
	...
	do{
		...
		for (int k = iVeicForn; k < limVeic; ++k, ++iVeicForn){
			...
			ElementarTW camElemForn(G, true, valorSubtrair);
		...
		for (int k = iVeicCons; k < limVeic; ++k, ++iVeicCons){
			...
			ElementarTW camElemForn(G, false, valorSubtrair);
			if (camElemForn.calculaCaminhoElementar(G, inicioJanelaCons, fimJanelaDeposito) > 0){
				colunas = camElemForn.getRotaCustoMinimo(G, 0.7);
				for (int i = 0; colunas[i] != NULL; ++i){
...
bool NoArvore::verificaFilhoViavel(Grafo* g, int iFilho){
	...
	if ((colunaOK) && (iFilho != 2)){
		int cargaRotaFornecedores = 0, cargaRotaConsumidores = 0, capacidade = g->getCapacVeiculo();
		for (int q = 1; q <= numCommod; ++q){
