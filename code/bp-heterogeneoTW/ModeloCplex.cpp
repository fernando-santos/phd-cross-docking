...
: env(), modelPrimal(env), cplexPrimal(modelPrimal),  modelDual(env), cplexDual(modelDual), costDual(env),
  maxVeic(maxVeiculos), nCommodities(numCommodities), custoTrocaCD(custoTroca) {
	...
	for (int k = 0; k < maxVeic; ++k){
		qRotasForn[k] = nRotasF;
		qRotasCons[k] = nRotasC;
	}
...
void ModeloCplex::setA_qr(Rota** rForn){
	...
	for (int k = 0; k < maxVeic; ++k){
		for (int i = 0; i < qRotasForn[k]; ++i){
			...
			tam = v.size()-1;
			for(int j = 1; j < tam; ++j){
				A_qr[k][i][v[j]] = 1; //coloca-se 1 apenas naquelas posicoes em que o vertice esteja na rota
			}
			ptrRotasForn[k].push_back(rForn[i]);
...
void ModeloCplex::setB_qr(Rota** rCons){
	...
	for (int k = 0; k < maxVeic; ++k){
		for (int i = 0; i < qRotasCons[k]; ++i){
			...
			tam = v.size()-1;
			for(int j = 1; j < tam; ++j){
				B_qr[k][i][v[j]-nCommodities] = 1; //coloca-se 1 apenas naquelas posicoes em que o vertice esteja na rota
			}
			ptrRotasCons[k].push_back(rCons[i]);
