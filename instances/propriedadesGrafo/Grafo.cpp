...
int Grafo::getPosX(int i){
	return vert[i]->posX;
}

int Grafo::getPosY(int i){
	return vert[i]->posY;
}
...
Grafo::Grafo(char* instanciaSolomon, int nCmdt) : numCmdt(nCmdt), numVertices(2*nCmdt + 2){
	...
	for (int i = 0; i < numVertices-1; ++i){
		inFile >> tmp >> matriz[i][0] >> matriz[i][1] >> vert[i]->pesoCommodity >> tmp >> tmp >> tmp;
		vert[i]->posX = matriz[i][0];
		vert[i]->posY = matriz[i][1];
	}
	inFile.close();
